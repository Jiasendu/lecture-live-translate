...
- 🚀 **API 版**：OpenAI 语音识别（实时 / 省钱两档），GPT 或 Claude 结合前文翻译，可填写课程主题和专业词，自动把碎句接起来
...
| 识别模型 | 效果 | 识别费用 |
| --- | --- | --- |
| 实时 `gpt-live-transcribe` | 边说边出字 | 约 $1.5 |
| 省钱 `gpt-transcribe` | 一句说完才出字 | 约 $0.4 |
| 最省 `gpt-4o-mini-transcribe` | 一句说完才出字，略不准 | 约 $0.3 |

翻译默认用 `gpt-6-luna`，一节课约几美分；换成 `gpt-6-sol` 更准，但约贵 20 倍。页面右上角会实时显示本次用时和花费估算。

被停顿切碎的短句（比如老师列举时的一个个词）会自动接回上一行，整段重新翻译，读起来更连贯。
